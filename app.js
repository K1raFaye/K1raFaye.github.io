class CQBTacticalBoard {
    ...
    showSceneActionSheet(layerId, sceneId) {
        const layer = this.layers.find(l => l.id === layerId);
        const scene = layer ? layer.scenes.find(s => s.id === sceneId) : null;
        if (!scene) return;

        const existing = document.getElementById('sceneActionSheet');
        if (existing) existing.remove();

        const sheet = document.createElement('div');
        sheet.id = 'sceneActionSheet';
        sheet.innerHTML = `
            <div class="sheet-overlay"></div>
            <div class="sheet-panel">
                <div class="sheet-title">${scene.name}</div>
                <div class="sheet-actions">
                    <button class="sheet-btn sheet-rename" data-action="rename">✏️ 重命名</button>
                    <button class="sheet-btn sheet-delete" data-action="delete" ${layer.scenes.length <= 1 ? 'disabled style="opacity:0.3;pointer-events:none"' : ''}>🗑️ 删除场景</button>
        ...
        sheet.querySelectorAll('.sheet-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                if (action === 'rename') {
                    close();
                    setTimeout(() => {
                        const tab = document.querySelector(`.scene-tab[data-scene="${sceneId}"]`);
                        if (tab) {
                    ...
                    close();
                    this.deleteScene(layerId, sceneId);
                } else {
                    close();
                }
            });
        ...
        this.layers.forEach((layer, index) => {
            ...
            const sceneTabsHtml = layer.scenes.map(scene => 
                `<span class="scene-tab${scene.id === layer.currentSceneId ? ' active' : ''}" data-scene="${scene.id}" title="${scene.name} - 双击重命名，悬停删除">
                    ...
                    <span class="scene-tab-del" data-scene-del="${scene.id}" title="删除此场景">×</span>
                </span>`
            ...
            item.querySelectorAll('.scene-tab').forEach(tab => {
                ...
                tab.addEventListener('touchstart', (e) => {
                    longPressTimer = setTimeout(() => {
                        this.showSceneActionSheet(layer.id, sceneId);
                        longPressTimer = null;
                    }, 500);
                }, { passive: true });
        ...
        if (touch.clientX >= rect.left && touch.clientX <= rect.right &&
            ...
            if (e.touches.length === 1) {
                ...
                if (clickedElement && (clickedElement.type === 'operator' || clickedElement.type === 'hostile')) {
                    ...
                    this.dragStartX = x - clickedElement.x;
                    this.dragStartY = y - clickedElement.y;
                } else if (this.currentTool === 'draw' || this.currentTool.startsWith('draw-')) {
                    // 画板工具 → 开始绘制
                    this.deselectElement();
                    if (this.currentTool === 'draw-erase-area') {
                        this.isErasingArea = true;
    ...
    handleTouchMove(e) {
        ...
        if (e.touches.length === 2) {
            ...
            if (this.initialPinchDist > 0) {
                const newScale = this.initialPinchScale * (currentDist / this.initialPinchDist);
    ...
    handleTouchEnd(e) {
        ...
        if (e.touches.length === 0 && e.changedTouches.length === 1) {
            ...
            if (touch.clientX >= rect.left && touch.clientX <= rect.right &&
                ...
                const pos = this.screenToCanvas(touch.clientX - rect.left, touch.clientY - rect.top);
                ...
                if (this.isDrawingPath) {
                    ...
                    if (scene) {
                        if (this.currentTool === 'draw') {
                            ...
                            const dx = Math.min(this.drawStartX, pos.x);
                            const dy = Math.min(this.drawStartY, pos.y);
                            const dw = Math.abs(pos.x - this.drawStartX);
                            const dh = Math.abs(pos.y - this.drawStartY);
                            if (dw > 5 && dh > 5) {
                                scene.elements.push({
                                    id: this.idCounter++,
                                    type: 'draw-rect',
                            ...
                            const radius = Math.sqrt(
                                Math.pow(pos.x - this.drawStartX, 2) + Math.pow(pos.y - this.drawStartY, 2)
                            );
                            if (radius > 5) {
                                scene.elements.push({
                                    id: this.idCounter++,
                            ...
                            const dx = pos.x - this.drawStartX;
                            const dy = pos.y - this.drawStartY;
                            const length = Math.sqrt(dx * dx + dy * dy);
                            if (length > 5) {
                                scene.elements.push({
                                    id: this.idCounter++,
                        ...
                        } else if (this.currentTool === 'draw-erase-area') {
                            const eraseArea = {
                                x: Math.min(this.eraseStartX, pos.x),
                                y: Math.min(this.eraseStartY, pos.y),
                                width: Math.abs(pos.x - this.eraseStartX),
                                height: Math.abs(pos.y - this.eraseStartY)
                            };
                            const elementsToKeep = scene.elements.filter(el => {
                                if (!el.type.startsWith('draw-')) return true;
                                if (el.type === 'draw-rect') return !this.rectIntersects({ x: el.x, y: el.y, width: el.width, height: el.height }, eraseArea);
                                else if (el.type === 'draw-circle') return !this.circleIntersectsRect(el, eraseArea);
                                else if (el.type === 'draw-line') return !this.lineIntersectsRect(el, eraseArea);
                                else if (el.type === 'draw-path') return !this.pathIntersectsRect(el, eraseArea);
                                return true;
                            });
                            if (elementsToKeep.length !== scene.elements.length) {
                                scene.elements = elementsToKeep;
    ...
    drawLayerInfo() {
        ...
        this.ctx.strokeRect(10, 10, 150, 26);

        this.ctx.fillStyle = '#00d9ff';
        this.ctx.font = 'bold 12px Arial';
        this.ctx.textAlign = 'left';
        this.ctx.textBaseline = 'middle';
        this.ctx.fillText(` ${currentLayer.name}`, 18, 23);
        this.ctx.fillText(` ${sceneName}`, 90, 23);
