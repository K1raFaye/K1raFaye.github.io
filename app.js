class CQBTacticalBoard {
    constructor() {
        ...
        // 多楼层系统
        this.layers = [{ id: 0, name: '1F 一楼', scenes: [{ id: 0, name: '场景1', elements: [], gunlines: [] }], currentSceneId: 0, sceneCounter: 1 }];
        ...
        // 人员图标预加载
        this.personIcons = {};
        this.personIconsLoaded = false;
        this.loadPersonIcons();
        
        // 实战模式
        this.combatMode = false;
        this.combatModeTimer = null;
    ...
    loadPersonIcons() {
        const iconMap = { operator: 'user', hostile: 'skull', hostage: 'user_alert' };
        let loaded = 0;
        const total = Object.keys(iconMap).length;
        for (const [type, name] of Object.entries(iconMap)) {
            const img = new Image();
            img.onload = () => {
                loaded++;
                if (loaded >= total) {
                    this.personIconsLoaded = true;
                }
            };
            img.src = `icons/${name}_64px.png`;
            this.personIcons[type] = img;
        }
    }
    ...
    get currentElements() {
        ...
        if (!layer) return [];
        const scene = layer.scenes.find(s => s.id === layer.currentSceneId);
    ...
    bindEvents() {
        ...
        document.getElementById('exportWorkBtn').addEventListener('click', () => this.exportWork());
        document.getElementById('importWorkBtn').addEventListener('click', () => this.importWork());
        document.getElementById('importWorkFile').addEventListener('change', (e) => {
        ...
        document.getElementById('combatModeToggle').addEventListener('change', (e) => {
            ...
            clearTimeout(this.combatModeTimer);
            if (this.combatMode) {
                indicator.classList.remove('fade-out');
                indicator.style.display = 'block';
                document.getElementById('combatHint').textContent = '(已启用)';
                this.combatModeTimer = setTimeout(() => {
                ...
                indicator.style.display = 'none';
                document.getElementById('combatHint').textContent = '(敌方隐藏)';
            }
            this.render();
        });
    ...
    saveHistory() {
        const snapshot = JSON.stringify(this.layers.map(layer => ({
            id: layer.id,
            name: layer.name,
            scenes: JSON.parse(JSON.stringify(layer.scenes)),
            currentSceneId: layer.currentSceneId,
            sceneCounter: layer.sceneCounter
        })));
    ...
    undo() {
        ...
        prev.forEach(data => {
            ...
            if (layer) {
                ...
                layer.scenes = data.scenes;
                layer.currentSceneId = data.currentSceneId;
                layer.sceneCounter = data.sceneCounter;
            }
        });
    ...
    redo() {
        ...
        data.forEach(d => {
            ...
            if (layer) {
                layer.name = d.name;
                layer.scenes = d.scenes;
                layer.currentSceneId = d.currentSceneId;
                layer.sceneCounter = d.sceneCounter;
            }
        });
    ...
    getCurrentScene(layer) {
        if (!layer) return null;
        return layer.scenes.find(s => s.id === layer.currentSceneId);
    }
    ...
    addScene(layerId) {
        const layer = this.layers.find(l => l.id === layerId);
        if (!layer) return;
        const prevScene = layer.scenes.find(s => s.id === layer.currentSceneId);
        layer.sceneCounter++;
        const newScene = {
            id: Date.now(),
            name: `场景${layer.sceneCounter}`,
            elements: prevScene ? JSON.parse(JSON.stringify(prevScene.elements)) : [],
            gunlines: prevScene ? JSON.parse(JSON.stringify(prevScene.gunlines)) : []
        };
        layer.scenes.push(newScene);
        layer.currentSceneId = newScene.id;
        this.deselectElement();
        this.updateLayerList();
        this.saveHistory();
        this.render();
    ...
    switchScene(layerId, sceneId) {
        const layer = this.layers.find(l => l.id === layerId);
        if (!layer) return;
        const scene = layer.scenes.find(s => s.id === sceneId);
        if (!scene) return;
        layer.currentSceneId = sceneId;
        this.deselectElement();
        this.updateLayerList();
        this.render();
    }

    deleteScene(layerId, sceneId) {
        const layer = this.layers.find(l => l.id === layerId);
        if (!layer || layer.scenes.length <= 1) return;
        const index = layer.scenes.findIndex(s => s.id === sceneId);
        if (index === -1) return;
        layer.scenes.splice(index, 1);
        if (layer.currentSceneId === sceneId) {
            layer.currentSceneId = layer.scenes[Math.max(0, index - 1)].id;
        }
        this.deselectElement();
        this.updateLayerList();
        this.saveHistory();
    ...
    renameScene(layerId, sceneId, newName) {
        const layer = this.layers.find(l => l.id === layerId);
        if (!layer) return;
        const scene = layer.scenes.find(s => s.id === sceneId);
    ...
    updateLayerList() {
        ...
        this.layers.forEach((layer, index) => {
            ...
            const sceneTabsHtml = layer.scenes.map(scene => 
                `<span class="scene-tab${scene.id === layer.currentSceneId ? ' active' : ''}" data-scene="${scene.id}" title="${scene.name} - 双击重命名，悬停删除">
                    <span class="scene-tab-name">${scene.name}</span>
                    <span class="scene-tab-del" data-scene-del="${scene.id}" title="删除此场景"><img src="icons/cross.svg" class="tool-icon" alt=""></span>
                </span>`
            ).join('') + `<button class="scene-add-btn" data-layer="${layer.id}" title="新增场景">+</button>`;
            
            item.innerHTML = `
                <div class="layer-header">
                    <span class="layer-name" contenteditable="true" title="点击编辑名称">${layer.name}</span>
                    <div class="layer-actions">
                        <button class="layer-up" data-action="up" title="上移"><img src="icons/arrow_up.svg" class="tool-icon" alt=""></button>
                        <button class="layer-down" data-action="down" title="下移"><img src="icons/arrow_down.svg" class="tool-icon" alt=""></button>
                        <button class="layer-delete" data-delete="${layer.id}" title="删除此楼层"><img src="icons/cross.svg" class="tool-icon" alt=""></button>
                    </div>
                </div>
                <div class="scene-tabs">${sceneTabsHtml}</div>
            `;
            ...
            item.querySelectorAll('.scene-tab').forEach(tab => {
                const sceneId = parseInt(tab.dataset.scene);
                let clickTimer = null;

                tab.addEventListener('click', (e) => {
                    if (e.target.classList.contains('scene-tab-del')) return;
                    if (tab.querySelector('.scene-tab-name[contenteditable="true"]')) return;
                    e.stopPropagation();
                    clearTimeout(clickTimer);
                    clickTimer = setTimeout(() => {
                        this.switchScene(layer.id, sceneId);
                    }, 220);
                });
                ...
                tab.addEventListener('dblclick', (e) => {
                    e.stopPropagation();
                    clearTimeout(clickTimer);
                    const nameSpan = tab.querySelector('.scene-tab-name');
                ...
                tab.addEventListener('contextmenu', (e) => {
                    ...
                    if (layer.scenes.length > 1) {
                        this.deleteScene(layer.id, sceneId);
                    }
                });
                ...
                nameSpan.addEventListener('blur', (e) => {
                    ...
                    this.renameScene(layer.id, sceneId, e.target.textContent);
                });
                nameSpan.addEventListener('keydown', (e) => {
                    ...
                    if (e.key === 'Escape') {
                        e.preventDefault();
                        nameSpan.textContent = layer.scenes.find(s => s.id === sceneId)?.name || '';
                        nameSpan.blur();
                    }
                });

                const delBtn = tab.querySelector('.scene-tab-del');
            ...
            item.querySelector('.scene-add-btn').addEventListener('click', (e) => {
                e.stopPropagation();
                this.addScene(layer.id);
            });
    ...
    clearCurrentLayer() {
        if (confirm(`确定要清空当前场景的所有元素吗？此操作不可撤销。`)) {
            const layer = this.layers.find(l => l.id === this.currentLayerId);
            const scene = layer ? layer.scenes.find(s => s.id === layer.currentSceneId) : null;
            if (scene) {
                scene.elements = [];
                scene.gunlines = [];
                this.deselectElement();
    ...
    snapDimension(rawWidth, rawHeight) {
        if (!this.snapEnabled) return { w: rawWidth, h: rawHeight, snapped: false };

        const dims = this.getExistingDimensions();
        const dimThreshold = 8;

        let snappedW = rawWidth;
        let snappedH = rawHeight;
        let info = '';

        for (const d of dims) {
            if (Math.abs(rawWidth - d) <= dimThreshold) {
                snappedW = d;
                info += info ? `, W=${d}` : `W=${d}`;
                break;
            }
        ...
        for (const d of dims) {
            if (Math.abs(rawHeight - d) <= dimThreshold) {
                snappedH = d;
                info += info ? `, H=${d}` : `H=${d}`;
                break;
            }
        ...
        if (info) {
            this.showSnapIndicator(info);
        }

        return { w: snappedW, h: snappedH, snapped: !!info };
    }

    getExistingDimensions() {
        const dims = new Set();
        for (const el of this.currentElements) {
            if (['wall', 'door', 'window', 'stairs', 'obstacle'].includes(el.type)) {
                if (el.width > 5) dims.add(Math.round(el.width));
                if (el.height > 5) dims.add(Math.round(el.height));
            }
        }
    ...
    handleContextMenu(e) {
        ...
        const rect = this.canvas.getBoundingClientRect();
        const pos = this.screenToCanvas(e.clientX - rect.left, e.clientY - rect.top);
        const x = pos.x;
        const y = pos.y;
    ...
    handleMouseMove(e) {
        ...
        // 画布平移中
        if (this.isPanning) {
            this.panX = this.lastPanX + (screenX - this.panStartX);
            this.panY = this.lastPanY + (screenY - this.panStartY);
            this.render();
            return;
        }
    ...
    handleMouseUp(e) {
        ...
        if (e.button === 2) {
            if (this.isDrawingGunline && this.gunlineSourcePerson && this.gunlinePreviewEnd) {
                const scene = this.getCurrentScene(this.layers.find(l => l.id === this.currentLayerId));
                if (scene) {
                    scene.gunlines.push({
                        id: this.idCounter++,
        ...
        if (this.isErasingArea) {
            ...
            if (scene) {
                ...
                const elementsToKeep = scene.elements.filter(el => {
                ...
                if (elementsToKeep.length !== scene.elements.length) {
                    scene.elements = elementsToKeep;
                    this.saveHistory();
                }
            }
    ...
    showTooltip(person, x, y) {
        ...
        const typeNames = { operator: '<img src="icons/user.svg" class="tool-icon" alt=""> 操作员', hostile: '<img src="icons/skull.svg" class="tool-icon" alt=""> 敌人', hostage: '<img src="icons/user_alert.svg" class="tool-icon" alt=""> 人质' };
        header.innerHTML = typeNames[person.type] || person.type;
    ...
    createElement(x1, y1, x2, y2) {
        ...
        // 建筑元素尺寸吸附
        if (this.snapEnabled && ['wall', 'door', 'window', 'stairs', 'obstacle'].includes(this.currentTool)) {
            const dimSnap = this.snapDimension(width, height);
            width = dimSnap.w;
            height = dimSnap.h;
        }
    ...
    deleteSelectedGunline() {
        ...
        if (idx !== undefined) {
            const scene = this.getCurrentScene(this.layers.find(l => l.id === this.currentLayerId));
            if (scene && scene.gunlines[idx]) {
                scene.gunlines.splice(idx, 1);
                this.saveHistory();
                this.deselectElement();
            }
        }
    ...
    exportWork() {
        ...
        const link = document.createElement('a');
        link.download = `cqb-work-${Date.now()}.json`;
        link.href = url;
        link.click();
    ...
    importWork() {
        document.getElementById('importWorkFile').click();
    }
    ...
    handleImportFile(file) {
        ...
        reader.onload = (e) => {
            try {
                ...
                if (!workData.layers || !Array.isArray(workData.layers)) {
                    alert('[ERROR] 无效的工作文件：缺少 floor 数据。');
                    return;
                }

                this.layers = workData.layers;
                this.currentLayerId = workData.currentLayerId ?? this.layers[0]?.id ?? 0;
                this.layerCounter = workData.layerCounter ?? this.layers.length;
                this.idCounter = workData.idCounter ?? this.idCounter;
                this.globalSettings = workData.globalSettings ?? { operatorSize: 18 };
                this.combatMode = workData.combatMode ?? false;
                this.showGrid = workData.showGrid ?? true;
                this.snapEnabled = workData.snapEnabled ?? true;
                this.occlusionEnabled = workData.occlusionEnabled ?? true;

                // 更新UI
                document.getElementById('combatModeToggle').checked = this.combatMode;
                document.getElementById('snapToggle').checked = this.snapEnabled;
                document.getElementById('occlusionToggle').checked = this.occlusionEnabled;
                document.getElementById('gridToggle').classList.toggle('active', this.showGrid);
                document.getElementById('globalOperatorSize').value = this.globalSettings.operatorSize;
                document.getElementById('operatorSizeValue').textContent = this.globalSettings.operatorSize + 'px';
                ...
                this.layers.forEach(l => {
                    l.scenes.forEach(s => {
                        sceneCount++;
                        elementCount += (s.elements?.length || 0) + (s.gunlines?.length || 0);
                    });
                });

                alert(`[OK] 工作文件导入成功！\n• ${layerCount} 个楼层\n• ${sceneCount} 个场景\n• ${elementCount} 个元素`);
            } catch (err) {
                alert('[ERROR] 无法解析工作文件：' + err.message);
            }
        };
    ...
    drawPreview(x, y) {
        ...
        if (['operator', 'hostile', 'hostage'].includes(this.currentTool)) {
            ...
            let w = x - this.drawStartX;
            let h = y - this.drawStartY;
            const dx = Math.min(this.drawStartX, x);
            const dy = Math.min(this.drawStartY, y);
            let dw = Math.abs(w);
            let dh = Math.abs(h);
            ...
            // 尺寸吸附
            const dimSnap = this.snapDimension(dw, dh);
            dw = dimSnap.w;
            dh = dimSnap.h;
            ...
            // 尺寸吸附（墙体、窗户、楼梯、障碍物）
            const dimSnap = this.snapDimension(Math.abs(w), Math.abs(h));
            w = (w >= 0 ? 1 : -1) * dimSnap.w;
            h = (h >= 0 ? 1 : -1) * dimSnap.h;
    ...
    drawLayerInfo() {
        ...
        const currentScene = currentLayer.scenes.find(s => s.id === currentLayer.currentSceneId);
        const sceneName = currentScene ? currentScene.name : '';

        const titleEl = document.getElementById('alertLayerName');
        const descEl = document.getElementById('alertSceneName');
        if (titleEl) titleEl.textContent = currentLayer.name;
        if (descEl) descEl.textContent = sceneName;
    }
    ...
    raycastRotatedDoor(originX, originY, dirX, dirY, doorEl) {
        ...
        const doorLen = isHorizontal ? doorEl.width : doorEl.height;
        const doorThick = isHorizontal ? doorEl.height : doorEl.width;

        // 根据铰链方向决定门板延伸方向
        const lenDir = (side === 'right' || side === 'bottom') ? -1 : 1;

        let corners;
        if (isHorizontal) {
            corners = [
                rotatePoint(hingeX, hingeY - doorThick / 2),
                rotatePoint(hingeX + lenDir * doorLen, hingeY - doorThick / 2),
                rotatePoint(hingeX + lenDir * doorLen, hingeY + doorThick / 2),
                rotatePoint(hingeX, hingeY + doorThick / 2)
            ];
        } else {
            corners = [
                rotatePoint(hingeX - doorThick / 2, hingeY),
                rotatePoint(hingeX - doorThick / 2, hingeY + lenDir * doorLen),
                rotatePoint(hingeX + doorThick / 2, hingeY + lenDir * doorLen),
                rotatePoint(hingeX + doorThick / 2, hingeY)
            ];
        }
    ...
    drawElement(el) {
        ...
        if (el.label) {
            ...
            this.ctx.font = 'bold 11px "Segoe UI","Helvetica Neue",Helvetica,Arial,sans-serif';
    ...
    drawDoor(el) {
        ...
        // 基于元素颜色的色板
        const baseColor = el.color || '#f39c12';
        ...
        if (isOpen) {
            ...
            if (isHorizontal) {
                corners = [
                    ...
                    rotPoint(lenDir * doorLen, -doorThick / 2),
                    rotPoint(lenDir * doorLen, doorThick / 2),
                    rotPoint(0, doorThick / 2)
                ];
            } else {
                corners = [
                    ...
                    rotPoint(-doorThick / 2, lenDir * doorLen),
                    rotPoint(doorThick / 2, lenDir * doorLen),
                    rotPoint(doorThick / 2, 0)
                ];
            }
            ...
            if (isFinite(corners[0].x) && isFinite(corners[0].y) &&
                ...
                grad.addColorStop(0, this.adjustColor(baseColor, 30));
                grad.addColorStop(0.5, baseColor);
                grad.addColorStop(1, this.adjustColor(baseColor, -30));
                fillStyle = grad;
            }
            this.ctx.fillStyle = fillStyle;
            this.ctx.fill();

            this.ctx.strokeStyle = this.adjustColor(baseColor, -50);
            this.ctx.lineWidth = 2;
            this.ctx.stroke();

            const handleLocal = isHorizontal
                ? { x: lenDir * (doorLen - 8), y: 0 }
                : { x: 0, y: lenDir * (doorLen - 8) };
            const handleWorld = rotPoint(handleLocal.x, handleLocal.y);

            this.ctx.fillStyle = this.adjustColor(baseColor, -60);
            this.ctx.beginPath();
            this.ctx.arc(handleWorld.x, handleWorld.y, 4, 0, Math.PI * 2);
            this.ctx.fill();

            this.ctx.strokeStyle = this.adjustColor(baseColor, -80);
            this.ctx.lineWidth = 1;
            this.ctx.stroke();

            const labelLocal = isHorizontal
                ? { x: lenDir * doorLen / 2, y: -doorThick / 2 - 12 }
                : { x: side === 'left' ? -16 : 16, y: lenDir * doorLen / 2 };
            ...
            grad.addColorStop(0, this.adjustColor(baseColor, 25));
            grad.addColorStop(0.5, baseColor);
            grad.addColorStop(1, this.adjustColor(baseColor, -25));
            this.ctx.fillStyle = grad;
            this.ctx.fillRect(el.x, el.y, el.width, el.height);

            this.ctx.strokeStyle = this.adjustColor(baseColor, -45);
            this.ctx.lineWidth = 2;
            this.ctx.strokeRect(el.x, el.y, el.width, el.height);

            this.ctx.fillStyle = this.adjustColor(baseColor, -55);
            this.ctx.beginPath();
            this.ctx.arc(el.x + el.width / 2, el.y + el.height / 2, 4, 0, Math.PI * 2);
            this.ctx.fill();

            this.ctx.strokeStyle = this.adjustColor(baseColor, -75);
            this.ctx.lineWidth = 1;
            this.ctx.stroke();
        }

        // 4. 铰链（金属质感）
        this.ctx.fillStyle = this.adjustColor(baseColor, -35);
        this.ctx.beginPath();
        this.ctx.arc(hingeX, hingeY, 6, 0, Math.PI * 2);
        this.ctx.fill();

        this.ctx.strokeStyle = this.adjustColor(baseColor, -65);
        this.ctx.lineWidth = 2;
        this.ctx.stroke();

        this.ctx.fillStyle = this.adjustColor(baseColor, -85);
        this.ctx.beginPath();
        ...
        this.ctx.font = 'bold 10px "Segoe UI","Helvetica Neue",Helvetica,Arial,sans-serif';
        this.ctx.textAlign = 'center';
        this.ctx.textBaseline = 'middle';
    ...
    drawPerson(el) {
        ...
        if (el.personName) {
            ...
            this.ctx.font = 'bold 9px "Segoe UI","Helvetica Neue",Helvetica,Arial,sans-serif';
            this.ctx.textAlign = 'center';
            this.ctx.textBaseline = 'top';
        ...
        // 绘制人员图标
        const personIcon = this.personIcons[el.type];
        if (personIcon && personIcon.complete && personIcon.naturalWidth > 0) {
            const iconSize = radius * 2;
            this.ctx.drawImage(personIcon, el.x - iconSize / 2, el.y - iconSize / 2, iconSize, iconSize);
        } else {
            // 回退：如果图标未加载，使用文字符号
            this.ctx.fillStyle = '#fff';
            this.ctx.font = `bold ${radius}px "Segoe UI","Helvetica Neue",Helvetica,Arial,sans-serif`;
            this.ctx.textAlign = 'center';
            this.ctx.textBaseline = 'middle';
    ...
    drawDrawPreview(x, y) {
        ...
        this.ctx.lineWidth = this.drawingWidth / this.scale;
    ...
    clearDrawElements() {
        const scene = this.getCurrentScene(this.layers.find(l => l.id === this.currentLayerId));
        if (scene) {
            const elementsToKeep = scene.elements.filter(el => !el.type.startsWith('draw-'));
            if (elementsToKeep.length !== scene.elements.length) {
                scene.elements = elementsToKeep;
                this.saveHistory();
                this.render();
    ...
    setupElasticSliders() {
        ...
        sliders.forEach(slider => {
            ...
            if (slider.parentElement.classList.contains('range-wrapper')) return;
            ...
            // Update tip position and value
            const updateTip = () => {
                const pct = (slider.value - min) / range;
            ...
            const updateTrackFill = () => {
                const pct = ((slider.value - min) / range) * 100;
                slider.style.background = `linear-gradient(to right, var(--foreground) 0%, var(--foreground) ${pct}%, rgba(128,128,128,0.4) ${pct}%, rgba(128,128,128,0.4) 100%)`;
            };
...
class ResizablePanels {
    constructor() {
        this.leftPanel = document.getElementById('leftToolbar');
        this.rightPanel = document.getElementById('rightPanel');
        this.leftHandle = document.getElementById('leftResizeHandle');
        this.rightHandle = document.getElementById('rightResizeHandle');
    ...
    init() {
        // Left handle (between toolbar and canvas)
        this.leftHandle.addEventListener('mousedown', (e) => this.onHandleDown(e, this.leftHandle, this.leftPanel));

        // Right handle (between canvas and properties panel)
        this.rightHandle.addEventListener('mousedown', (e) => this.onHandleDown(e, this.rightHandle, this.rightPanel));

        // Global move/up listeners
        document.addEventListener('mousemove', (e) => this.onHandleMove(e));
        document.addEventListener('mouseup', () => this.onHandleUp());

        // Persist sizes via localStorage
        this.restoreSizes();

        // Trigger canvas resize to match restored panel widths
        if (window.tacticalBoard) {
            window.tacticalBoard.resizeCanvas();
        }
    }

    onHandleDown(e, handle, panel) {
        e.preventDefault();
        this.activeHandle = handle;
        this.targetPanel = panel;

        this.startX = e.clientX;
        this.startWidth = panel.getBoundingClientRect().width;
    ...
    onHandleMove(e) {
        if (!this.activeHandle || !this.targetPanel) return;

        const clientX = e.clientX;
        const delta = clientX - this.startX;

        // For the left handle, dragging right → wider toolbar
        // For the right handle, dragging right → narrower properties panel (because we resize the panel itself)
        let newWidth;
        if (this.activeHandle === this.leftHandle) {
            // Left handle: drag right = wider left toolbar
            newWidth = this.startWidth + delta;
        } else {
            // Right handle: drag right = narrower right panel (move handle right, panel starts later)
            // The right panel is on the right edge; dragging the handle left = wider panel
            newWidth = this.startWidth - delta;
        }

        // Apply min/max constraints
        const minW = parseInt(getComputedStyle(this.targetPanel).minWidth) || 180;
        const maxW = parseInt(getComputedStyle(this.targetPanel).maxWidth) || 560;
        newWidth = Math.max(minW, Math.min(maxW, newWidth));

        // Apply
        this.targetPanel.style.width = newWidth + 'px';

        // Trigger canvas resize since available space changed
        if (window.tacticalBoard) {
            window.tacticalBoard.resizeCanvas();
        }
    }
    ...
    onHandleUp() {
        ...
        this.activeHandle.classList.remove('resizing');
    ...
    saveSizes() {
        try {
            const sizes = {
                left: this.leftPanel.style.width || getComputedStyle(this.leftPanel).width,
                right: this.rightPanel.style.width || getComputedStyle(this.rightPanel).width
            };
            localStorage.setItem('tactical-board-panel-sizes', JSON.stringify(sizes));
        } catch (_) { /* localStorage may be unavailable */ }
    }

    restoreSizes() {
        try {
            const raw = localStorage.getItem('tactical-board-panel-sizes');
            if (!raw) return;
            const sizes = JSON.parse(raw);
            if (sizes.left) this.leftPanel.style.width = sizes.left;
            if (sizes.right) this.rightPanel.style.width = sizes.right;
        } catch (_) { /* ignore */ }
    }
}


// 初始化应用（由 splash.js 在启动动画结束后调用，
// 或 DOMContentLoaded 时若主界面已可见则直接初始化）
function initApp() {
    if (window.tacticalBoard) return;
    window.tacticalBoard = new CQBTacticalBoard();
    window.resizablePanels = new ResizablePanels();
}
window.initCQBApp = initApp;

document.addEventListener('DOMContentLoaded', () => {
    const mainApp = document.getElementById('mainApp');
    // 如果没有 splash 包裹（mainApp 直接可见），则立即初始化
    if (!mainApp || mainApp.style.display !== 'none') {
        initApp();
    }
    // 否则由 splash.js 在动画结束后调用 window.initCQBApp()
});
